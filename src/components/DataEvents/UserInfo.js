...
const UserInfo = ()=>{
    ...
    const genre = JSON.parse(window.localStorage.getItem("userSelect"))
    return (
        <div style={{width:"30vw",minHeight:"40vh",background:'#5746EA',borderRadius:"12px",padding:"6px",display:"flex",gap:"12px"}}>
        <div>
            <img src={Profile} style={{height:"32vh",width:"10vw",position:"relative",top:"2vh"}} alt=''/>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
            ...
            <Card categories={genre} color={"#9F94FF"}/>
        </div>
        </div>
    )
}

const Card = ({color,categories})=>{
    return(
